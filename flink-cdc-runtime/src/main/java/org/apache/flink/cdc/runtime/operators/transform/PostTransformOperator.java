...
public class PostTransformOperator extends AbstractStreamOperator<Event>
    ...
    private transient Map<Tuple2<TableId, TransformProjection>, PostTransformProcessor>
    ...
    public static class Builder {
        ...
        public PostTransformOperator build() {
            return new PostTransformOperator(transformRules, schemaOperatorID, timezone);
        }
    }

    private PostTransformOperator(
            List<TransformRule> transformRules, OperatorID schemaOperatorID, String timezone) {
    ...
    private Optional<DataChangeEvent> processDataChangeEvent(DataChangeEvent dataChangeEvent)
        ...
        TableInfo tableInfo = getTableInfoFromSchemaEvolutionClient(tableId);
    ...
    private Optional<DataChangeEvent> processPostProjection(
            TableInfo tableInfo, DataChangeEvent dataChangeEvent) throws Exception {
        BinaryRecordData before = (BinaryRecordData) dataChangeEvent.before();
        BinaryRecordData after = (BinaryRecordData) dataChangeEvent.after();
        if (before != null) {
