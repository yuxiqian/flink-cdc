 ...
 *
 * <p>Configuration options shared across the relational CDC connectors which use a persistent
 * database schema history.
 *
 * <p>Added {@link #JMX_METRICS_ENABLED} option so Flink CDC can disable schema history JMX metrics
 * registration (which otherwise conflicts when multiple subtasks run in the same JVM).
 */
public abstract class HistorizedRelationalDatabaseConnectorConfig
    ...
    public static final Field JMX_METRICS_ENABLED =
            Field.create(SchemaHistory.CONFIGURATION_FIELD_PREFIX_STRING + "metrics.enabled")
                    .withDisplayName("Enable JMX history metrics")
