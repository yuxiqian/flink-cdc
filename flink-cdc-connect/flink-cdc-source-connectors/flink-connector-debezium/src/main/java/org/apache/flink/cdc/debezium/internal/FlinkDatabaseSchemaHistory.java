 ...
 *
 * <p>Copied from Debezium 2.7.4.Final: migrated from the removed {@code DatabaseHistory} interface
 * to {@code SchemaHistory} (with the related {@code SchemaHistoryException}/{@code
 * SchemaHistoryListener} renames). The {@code record(...)} table-changes overload gained a trailing
 * {@link Instant} timestamp parameter, and the {@code storeOnlyCapturedTables()}/{@code
 * skipUnparseableDdlStatements()} SPI methods were dropped from the interface. The instance-name
 * config key now uses the {@code schema.history.internal.} prefix so it survives Debezium's history
 * config subset filtering.
 */
